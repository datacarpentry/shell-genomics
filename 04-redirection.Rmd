...
```{bash}
cd ~/dc_workshop/data/untrimmed_fastq/
grep NNNNNNNNNN  SRR2584863_2.fastq
```
...
```{bash}
cd ~/dc_workshop/data/untrimmed_fastq/
grep -B1 -A2 NNNNNNNNNN  SRR2584863_2.fastq
```
...
1. Search for the sequence `GNATNACCACTTCC` in the ` SRR2584863_2.fastq` file.
...
## Solution  
1. `grep -B1 GNATNACCACTTCC  SRR2584863_2.fastq` 
...
```{bash}
cd ~/dc_workshop/data/untrimmed_fastq/
grep -B1 -A2 NNNNNNNNNN  SRR2584863_2.fastq > bad_reads.txt
```
...
How many sequences are there in ` SRR2584863_2.fastq`? Remember that every sequence is formed by four lines.

## Solution

```{bash}
cd ~/dc_workshop/data/untrimmed_fastq/
wc -l  SRR2584863_2.fastq
```
...
How many sequences in ` SRR2584863_2.fastq` contain at least 3 consecutive Ns?

## Solution

```{bash}
cd ~/dc_workshop/data/untrimmed_fastq/
grep NNN  SRR2584863_2.fastq > bad_reads.txt
wc -l bad_reads.txt
```
...
```{bash}
cd ~/dc_workshop/data/untrimmed_fastq/
grep -B1 -A2 NNNNNNNNNN SRR2584863_1.fastq > bad_reads.txt
wc -l bad_reads.txt
```
Here, the output of our second  call to `wc` shows that we no longer have any lines in our `bad_reads.txt` file. This is 
because the second file we searched (`SRR2584863_1.fastq`) does not contain any lines that match our
...
```{bash}
cd ~/dc_workshop/data/untrimmed_fastq/
grep -B1 -A2 NNNNNNNNNN  SRR2584863_2.fastq | less
```
...
```{bash}
cd ~/dc_workshop/data/untrimmed_fastq/
grep -B1 -A2 NNNNNNNNNN  SRR2584863_2.fastq | wc -l 
```
...
```{bash}
cd ~/dc_workshop/data/untrimmed_fastq/
grep -B1 -A2 NNNNNNNNNN  SRR2584863_2.fastq | grep -v '^--' bad_reads.txt
tail bad_reads.txt
```
...
as the variable to be used over each iteration. In our case `SRR2584863_1.fastq` and ` SRR2584863_2.fastq` will be substituted for `filename` 
...
```{bash}
cd ~/dc_workshop/data/untrimmed_fastq/
basename SRR2584863_1.fastq .fastq
```
...
```{bash}
cd ~/dc_workshop/data/untrimmed_fastq/
basename SRR2584863_1.fastq .fasta
```
