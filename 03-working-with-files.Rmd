...
```{bash}
cd ~/shell_data/untrimmed_fastq
ls *.fastq.gz | grep SRR098026  
```

lists only the file that starts with SRR098026    
...
| key     | action |
| ------- | ---------- |
| <kbdSpace</kbd| to go forward |
|  <kbdb</kbd   | to go backward |
|  <kbdg</kbd   | to go to the beginning |
|  <kbdG</kbd   | to go to the end |
|  <kbdq</kbd   | to quit |
...
```{bash, eval=FALSE, engine="sh"}
cd ~/dc_workshop/data/untrimmed_fast
mkdir backup
```
...
1. `rm -r backup`  
2. `cp SRR098026.fastq  SRR098026-backup.fastq ` and `cp SRR097977.fastq  SRR097977-backup.fastq .fastq`  
